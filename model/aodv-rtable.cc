...
RoutingTableEntry::RoutingTableEntry (Ptr<NetDevice> dev, Ipv4Address dst, bool vSeqNo, uint32_t seqNo,
                                      Ipv4InterfaceAddress iface, uint16_t hops, Ipv4Address nextHop, Time lifetime, int pos, int neg) :
  m_ackTimer (Timer::CANCEL_ON_DESTROY),
  m_validSeqNo (vSeqNo), m_seqNo (seqNo), m_hops (hops),
  m_lifeTime (lifetime + Simulator::Now ()), m_iface (iface), m_flag (VALID),
  m_reqCount (0), m_blackListState (false), m_blackListTimeout (Simulator::Now ()), m_pos (pos), m_neg(neg), 
  m_bel(computeBelief(pos,neg)), m_disbel(computeDisbelief(pos,neg)), m_unc(computeUncertainty(pos,neg))
{
...
void
RoutingTable::UpdateTrustValues (RoutingTableEntry & rt)
{
  NS_LOG_FUNCTION (this);
  rt.SetBelief(rt.computeBelief(rt.GetPositiveEvents(), rt.GetNegativeEvents()));
  rt.SetDisbelief(rt.computeDisbelief(rt.GetPositiveEvents(), rt.GetNegativeEvents()));
  rt.SetUncertainty(rt.computeUncertainty(rt.GetPositiveEvents(), rt.GetNegativeEvents()));
}
