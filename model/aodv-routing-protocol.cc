...
RoutingProtocol::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("DestinationOnly", "Indicates only the destination may respond to this RREQ.",
                   BooleanValue (false), //Edit: In Blackhole soln, this is true
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
 ...
 /* Check if the node is suppose to behave maliciously */
        if(IsMalicious || EnableWrmAttack)
          {//When malicious node receives packet it drops the packet.
                std :: cout <<"Launching Attack! Packet dropped . . . \n";
                NS_LOG_INFO("Launching Attack! Packet dropped . . .");
               return false; 
          }
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  uint64_t tsr = Simulator::Now ().GetNanoSeconds();
  //std::cout <<  "Time send req: " << tsr << "\n";
  rreqHeader.SetTimeSendReq (tsr); //wormhole modif added el
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  switch (tHeader.Get ())
    ...
    case AODVTYPE_RREQDSE:
      {
        RecvRequestDSE(packet, sender);
        break;
      }
	case AODVTYPE_DSEREQ:
      {
        SendRequestDSE(packet, receiver, sender);
        break;
      }
    }
}
...
uint32_t
RoutingProtocol::HashMHCTimes (uint32_t hash, int total) {
	std::stringstream ss;
	ss << hash;
	int x;
	for (x = 0; x < total; x++) {
...
void
RoutingProtocol::RequestDSE (Ipv4Address src, Ipv4Address destination, uint32_t hopcount) {
  
  DSEReq h;
  
  
  Ptr<Packet> packet = Create<Packet> ();
  h.SetReserved(hopcount);
  packet->AddHeader (h);
  TypeHeader typeHeader (AODVTYPE_RREQDSE);
  packet->AddHeader (typeHeader);

  
  Ptr<Socket> socket = FindSocketWithInterfaceAddress(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress (src),0));
...
void
RoutingProtocol::SendRequestDSE (Ptr<Packet> p, Ipv4Address src, Ipv4Address destination) {
  
  RreqDSE h;
  DSEReq dsereq;
  p->RemoveHeader(dsereq);
  TypeHeader typeHeader (AODVTYPE_RREQDSE);
  Ptr<Packet> packet = Create<Packet> ();
  
  h.SetSeed(2);
  uint32_t hash = HashMHCTimes(h.GetSeed(), NetDiameter);
  h.SetTopHash(hash);
  h.SetMaxHopCount(NetDiameter);
  h.SetHopCount(dsereq.GetReserved());
  packet->AddHeader (h);
  packet->AddHeader (typeHeader);
...
void
RoutingProtocol::RecvRequestDSE (Ptr<Packet> p, Ipv4Address address) {
  
  RreqDSE rreqDSE;
  p->RemoveHeader(rreqDSE);
  int mhc = rreqDSE.GetMaxHopCount();
  int hopcount = rreqDSE.GetHopCount();
  uint32_t hash = HashMHCTimes(rreqDSE.GetSeed(), mhc - hopcount);
  
  if (hash == rreqDSE.GetTopHash()) {
	  //NS_LOG_INFO("Hash equal!");
...
bool 
RoutingProtocol::CheckHash (RreqDSE *rreqDSE, RrepHeader *rrepHeader) 
{
  
  int mhc = rreqDSE->GetMaxHopCount();
  int hopcount = rrepHeader->GetHopCount();
  
  int dsehopcount = rreqDSE->GetHopCount();
  uint32_t hash;
  hash  = HashMHCTimes(rreqDSE->GetSeed(), dsehopcount);
  rreqDSE->SetSeed(hash);
  int thhashed = rreqDSE->GetTHHashed();
  
  if (thhashed != 1) {
	  hash = HashMHCTimes(rreqDSE->GetOSeed(), mhc);
	  rreqDSE->SetTopHash(hash);
	  rreqDSE->SetTHHashed(1);
  }	  
	  
  hash = HashMHCTimes(rreqDSE->GetSeed(), mhc - hopcount);
  
  if (hash != rreqDSE->GetTopHash()) {
	  return false;
  }
  else {
	  hash = HashMHCTimes(rreqDSE->GetSeed(), 1);
	  rreqDSE->SetSeed(hash);
  }
  rreqDSE->SetHopCount(0);
  return true;
}
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  NS_LOG_FUNCTION (this);
  RreqHeader rreqHeader;
  NS_LOG_INFO("Received RREQ from " << src << ".");
  ...
  uint64_t trr = Simulator::Now ().GetNanoSeconds();
  rreqHeader.SetTimeRecvReq (trr); //wormhole modif added el
  ...
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
		//DPH for Wormhole solution
      uint64_t tsr = rreqHeader.GetTimeSendReq();
      uint64_t trr = rreqHeader.GetTimeRecvReq();
      uint64_t PropTime = trr - tsr; //Propagation Time for DPH (added el)
      uint8_t H = rreqHeader.GetHopCount(); //Hop Count
      uint64_t DPH = PropTime / H;
      //std::cout << "DPH Value: "<< DPH << " -----------\n";
      
      if(DPH < 30000000){ // threshold = 50000000 ns
		m_routingTable.LookupRoute (origin, toOrigin);
		NS_LOG_DEBUG ("Send reply since I am the destination");
		NS_LOG_INFO("I am the destination. Sending RREP to non-wormhole nodes.");
		NS_LOG_INFO("My IP: " << receiver << " Destination: " << rreqHeader.GetDst());
          ...
          if (IsMalicious || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
            {
              ...
              */
              if(IsMalicious)
              {
                ...
                NS_LOG_INFO("I am the malicious node. Forge and send RREP with fake hop count.");
                SendReplyByIntermediateNode (falseToDst, toOrigin, rreqHeader.GetGratiousRrep ());
                 return;
              }
              /* Code for Blackhole Attack Simulation ends here */
              NS_LOG_INFO("I have a route to the destination. sending RREP.");
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      m_lastBcastTime = Simulator::Now ();
      NS_LOG_INFO("Broadcast received RREQ.");
      Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &RoutingProtocol::SendTo, this, socket, packet, destination); 
    }
}
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  Ptr<Packet> packet = Create<Packet> ();
  RreqDSE rreqDSE;
  uint32_t seed = m_uniformRandomVariable->GetInteger (0, NetDiameter);
  rreqDSE.SetSeed(seed);
  rreqDSE.SetOSeed(seed);
  rreqDSE.SetTHHashed(0);
  rreqDSE.SetMaxHopCount(NetDiameter);
  rreqDSE.SetHopCount(0);
  rreqDSE.SetTopHash(0);
  packet->AddHeader (rreqDSE);
  ...
  NS_ASSERT (socket);
  NS_LOG_INFO("Added Hash Chain values to RREP.");
  NS_LOG_INFO("Sending RREP to " << toOrigin.GetNextHop() << ".");
...
RoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep)
{
  ...
  rreqDSE.SetMaxHopCount(NetDiameter);
  rreqDSE.SetHopCount(0);
  uint32_t hash = HashMHCTimes(rreqDSE.GetSeed(), NetDiameter);
  rreqDSE.SetTopHash(hash);
  packet->AddHeader (rreqDSE);
  ...
  NS_LOG_INFO("Added Hash Chain values to RREP.");
  if (IsMalicious) 
	NS_LOG_INFO("Sending fake RREP to " << toOrigin.GetNextHop() << ".");
  else 
	NS_LOG_INFO("Sending RREP to " << toOrigin.GetNextHop() << ".");
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  bool checkhash = CheckHash(&rreqDSE, &rrepHeader);
  
  NS_LOG_LOGIC ("RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ());

  NS_LOG_INFO("Received RREP from " << sender);
  NS_LOG_INFO("Validating RREP hop count... ");
  if (checkhash == false) {
	  //NS_LOG_INFO("Hop count invalid! Dropping RREP.");
	  //return;
  }
  //NS_LOG_INFO("Hop count valid!");
  ...
  RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ dst, /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
                                          ...
                                          /*nextHop=*/ sender, /*lifeTime=*/ rrepHeader.GetLifeTime (), 0, 0);
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
       ...
       */
	   NS_LOG_INFO("Trust Values for " << sender); 
	   NS_LOG_INFO("Belief: " << newEntry.GetBelief());
	   NS_LOG_INFO("Disbelief: " << newEntry.GetDisbelief());
	   NS_LOG_INFO("Uncertainty: " << newEntry.GetUncertainty()); 
	   NS_LOG_INFO ("No problems with RREP received. +1 Positive Events.");
	   newEntry.SetPositiveEvents(toDst.GetPositiveEvents() + 1);
		if (!toDst.GetValidSeqNo ())
		{
		  m_routingTable.Update (newEntry);
	  ...
	  // (ii)the Destination Sequence Number in the RREP is greater than the node's copy of the destination sequence number and the known value is valid,
		...
		m_routingTable.UpdateTrustValues (newEntry);
		NS_LOG_INFO("New Trust Values for " << sender); 
	    NS_LOG_INFO("Belief: " << newEntry.GetBelief());
	    NS_LOG_INFO("Disbelief: " << newEntry.GetDisbelief());
	    NS_LOG_INFO("Uncertainty: " << newEntry.GetUncertainty()); 
    }
  else
    {
	  ...
	  newEntry.SetPositiveEvents(1);
	  NS_LOG_INFO("New route to " << dst << "added to routing table.");
      m_routingTable.AddRoute (newEntry);
      ...
      if (toDst.GetFlag () == IN_SEARCH)
        {
	      ...
	      NS_LOG_INFO("Disbelief: " << newEntry.GetDisbelief());
	      NS_LOG_INFO("Uncertainty: " << newEntry.GetUncertainty());
		  NS_LOG_INFO ("I am the RREQ Originator. Successful route request. +1 Positive Events.");
  ...
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  NS_LOG_INFO("Sending RREP to " << toOrigin.GetNextHop() << " (next hop).");
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), AODV_PORT));
}
  ...
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor))
  ...
  else
    {
      ...
      if(EnableWrmAttack && wrmDst==FirstEndOfWormTunnel)
      {
          ...
          toNeighbor.SetPositiveEvents(1);
      }
